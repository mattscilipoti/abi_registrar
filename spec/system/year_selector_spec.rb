...
RSpec.describe 'Year selector', type: :system do
  ...
  within('form') { find('input[type=submit], button[type=submit]').click }
    ...
    # The page should show links for the two years we created
    expect(page).to have_link('2025')
    expect(page).to have_link('2024')

    # Click the 2024 link and assert the page updates the year param
    click_link '2024'
  ...
  expect(active_texts).to include('2024')
  end
end
